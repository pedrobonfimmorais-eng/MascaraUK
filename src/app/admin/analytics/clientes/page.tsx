...
export const metadata: Metadata = { title: ta("analytics.nav.customers") };
...
export default async function CustomersAnalyticsPage({ searchParams }: CustomersAnalyticsPageProps) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <h1 className="text-2xl font-bold text-brand-secondary">{ta("analytics.nav.customers")}</h1>
      ...
      <section className="rounded-xl border border-gray-200 p-4">
        <dl className="grid grid-cols-2 gap-3 text-sm sm:grid-cols-3 lg:grid-cols-4">
          <Item label={ta("analytics.metrics.totalCustomers")} value={String(summary.totalCustomers)} />
          <Item label={ta("analytics.metrics.newCustomers")} value={String(summary.newCustomers)} />
          <Item label={ta("analytics.customers.oneTime")} value={String(summary.oneTimeCustomers)} />
          <Item label={ta("analytics.customers.moreThanOnce")} value={String(summary.moreThanOnceCustomers)} />
          <Item label={ta("analytics.customers.withoutOrders")} value={String(summary.customersWithoutOrders)} />
          <Item label={ta("analytics.customers.avgSpendPerCustomer")} value={formatCurrency(summary.averageSpendPerCustomer)} />
          <Item label={ta("analytics.customers.avgOrdersPerCustomer")} value={summary.averageOrdersPerCustomer.toString()} />
          <Item
            label={ta("analytics.customers.avgDaysBetweenPurchases")}
            value={summary.averageDaysBetweenPurchases == null ? ta("analytics.insufficientData") : `${summary.averageDaysBetweenPurchases}`}
          />
        </dl>
        <p className="mt-2 text-xs text-gray-500">{ta("analytics.customers.recurringDefinition")}</p>
      </section>
          ...
          <table className="w-full text-left text-sm">
            <thead className="bg-gray-50 text-xs uppercase text-gray-500">
              <tr>
                ...
                <th className="px-4 py-3">{ta("account.myOrders")}</th>
                <th className="px-4 py-3">{ta("analytics.customers.totalSpent")}</th>
                <th className="px-4 py-3">{ta("analytics.customers.lastPurchase")}</th>
              </tr>
            </thead>
