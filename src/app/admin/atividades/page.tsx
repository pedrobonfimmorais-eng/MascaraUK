...
export const metadata: Metadata = { title: ta("admin.activities.title") };
...
export default async function AdminActivitiesPage({ searchParams }: AdminActivitiesPageProps) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <h1 className="text-2xl font-bold text-brand-secondary">{ta("admin.activities.title")}</h1>
      <p className="text-xs text-gray-500">{ta("admin.activities.readOnlyNote")}</p>
      ...
      <form className="flex flex-wrap gap-3 rounded-xl border border-gray-200 p-4 text-sm" method="get">
        <select name="action" defaultValue={action ?? ""} className="rounded-lg border border-gray-300 px-3 py-2">
          <option value="">{ta("admin.activities.allActions")}</option>
          {actions.map((value) => (
      ...
      {!logs || logs.length === 0 ? (
        <EmptyState title={ta("admin.activities.empty")} />
      ) : (
        <div className="overflow-x-auto rounded-xl border border-gray-200">
          <table className="w-full text-left text-sm">
            <thead className="bg-gray-50 text-xs uppercase text-gray-500">
              <tr>
                <th className="px-4 py-3">{ta("admin.activities.columnDate")}</th>
                <th className="px-4 py-3">{ta("admin.activities.columnUser")}</th>
                <th className="px-4 py-3">{ta("admin.activities.columnAction")}</th>
                <th className="px-4 py-3">{ta("admin.activities.columnEntity")}</th>
                <th className="px-4 py-3">{ta("admin.activities.columnDetails")}</th>
              </tr>
            </thead>
