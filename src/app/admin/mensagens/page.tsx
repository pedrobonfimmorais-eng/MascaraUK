...
export const metadata: Metadata = { title: ta("admin.messages.title") };
...
export default async function AdminMessagesPage({ searchParams }: AdminMessagesPageProps) {
  ...
  return (
    <div className="flex flex-col gap-6">
      <h1 className="text-2xl font-bold text-brand-secondary">{ta("admin.messages.title")}</h1>
      ...
      <form className="flex flex-wrap gap-3 rounded-xl border border-gray-200 p-4 text-sm" method="get">
        <select name="status" defaultValue={status ?? ""} className="rounded-lg border border-gray-300 px-3 py-2">
          <option value="">{ta("admin.messages.allStatuses")}</option>
          {STATUSES.map((value) => (
      ...
      {!messages || messages.length === 0 ? (
        <EmptyState title={ta("admin.messages.empty")} />
      ) : (
        <div className="overflow-x-auto rounded-xl border border-gray-200">
          <table className="w-full text-left text-sm">
            <thead className="bg-gray-50 text-xs uppercase text-gray-500">
              <tr>
                <th className="px-4 py-3">{ta("admin.messages.columnFrom")}</th>
                <th className="px-4 py-3">{ta("admin.messages.columnSubject")}</th>
                <th className="px-4 py-3">{ta("admin.messages.columnDate")}</th>
                <th className="px-4 py-3">{ta("admin.messages.columnStatus")}</th>
              </tr>
            </thead>
                ...
                <tr key={message.id} className="hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3">
                    <Badge tone={message.status === "nova" ? "brand" : message.status === "spam" ? "danger" : "neutral"}>
                      {ta(`admin.messages.status.${message.status}` as TranslationKey)}
                    </Badge>
                  </td>
                </tr>
              ))}
